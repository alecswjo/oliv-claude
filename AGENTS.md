...
- The LLM key is **server-side** in `supabase/functions/analyze` (OpenAI gpt-5.5, provider-pluggable). The app calls the proxy via `ProxyMealAnalyzer`; precedence is `proxy → offline estimator` (the in-app Codex analyzer was removed). Meals carry up to 5 photos (`Meal.photoUris`, DB `photo_paths text[]`). `supabase/` is excluded from the app `tsconfig` (it's Deno).
